...
int check_delta[(sizeof(int) << 3) + 1][2];
int avg_a;
...
void dec_dt(size_t x, int i){
    ...
    if(x == 0) cg[i][0] = 0, cg[i][1] = 0;
        ...
        int bin_cnt = 64 - (__builtin_clzll(x));
        ...
        if(bin_cnt - zero_suf_cnt != __builtin_popcountll(x)) {
            ...
            cg[i][0] = bin_cnt + 1;// 1 << bin_cnt
            cg[i][1] = zero_suf_cnt + 1;// 1 << zero_suf_cnt
        } 
    }    
}
...
void solve(){
    cin>>n;
    rec_break = false;
    memset(check_delta, 0, sizeof(check_delta));
    int sum_a = 0;
    for(size_t i = 1; i<=n; ++i){
        cin>>a[i];
        sum_a += a[i];
